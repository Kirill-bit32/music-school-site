...
async function setupTrialPage() {
  ...
  form.addEventListener("submit", async (e) => {
    ...
    try {
      ...
      showAlert("success", "Заявка на пробный урок отправлена! Мы свяжемся с вами в течение 30 минут.");
...
async function setupAdminPage() {
  ...
  if (clearAllBtn) {
    clearAllBtn.addEventListener("click", async () => {
      ...
      const promises = snap.docs.map(d => deleteDoc(doc(db, TRIALS_COLLECTION, d.id)));
  ...
  adminContent.addEventListener("click", (e) => {
    ...
    else if (["callback", "noanswer", "confirmed", "cancelled"].includes(action)) {
      setStatus(id, action);
    }
  });
